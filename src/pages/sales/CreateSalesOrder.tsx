...
export const CreateSalesOrder: React.FC = () => {
  ...
  const [items, setItems] = useState<SalesOrderItem[]>([
    { id: '1', description: '', quantity: 1, rate: 0, unit: 'pcs' },
  ]);
  ...
  const addItem = () => {
    setItems([...items, { id: Date.now().toString(), description: '', quantity: 1, rate: 0, unit: 'pcs' }]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Sales Order Data:', { ...formData, items, subtotal, total });
    // In a real app, this would be an API call
    // For now, we'll just simulate success
    alert(t('sales.success.salesOrderCreated') || 'Sales Order created successfully');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t('sales.salesOrders.create')}</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('sales.salesOrders.table.date')}</label>
            <input
              type="date"
              name="date"
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('sales.salesOrders.table.deliveryDate')}</label>
            <input
              type="date"
              name="deliveryDate"
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
                    ...
                    onChange={(e) => {
                      ...
                      if (product) {
                        setItems(items.map(i => 
                          i.id === item.id 
                            ? { ...i, productId: product.id, description: product.name, rate: product.unit_price, unit: product.unit || 'pcs' } 
                            : i
                        ));
                      } else {
