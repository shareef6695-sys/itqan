...
export const CreateBill: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Bill Data:', { vendorId, billDate, dueDate, reference, items, subtotal, total, notes });
    alert(t('purchase.success.billCreated'));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('purchase.bills.create')}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('purchase.vendors.form.vendor') || t('purchase.bills.table.vendor')}</label>
            <select
              ...
              required
            >
              <option value="">{t('purchase.vendors.form.selectVendor') || 'Select Vendor'}</option>
              {MOCK_VENDORS.map(v => (
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('purchase.bills.print.billDate')}</label>
            <input
              type="date"
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
              value={billDate}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('purchase.bills.print.dueDate')}</label>
            <input
              type="date"
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
              value={dueDate}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('purchase.bills.table.ref') || 'Reference'}</label>
            <input
              type="text"
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
        ...
        {/* Bill Items */}
        <div className="mt-8">
          <h3 className="text-lg font-medium text-gray-900 mb-4">{t('purchase.bills.print.itemDescription') || 'Items'}</h3>
            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-center space-x-4 rtl:space-x-reverse">
                <div className="w-48">
                  ...
                  >
                    <option value="">{t('purchase.purchaseOrders.form.selectProduct') || 'Select Product'}</option>
                    {MOCK_PRODUCTS.map(p => (
                ...
                <div className="flex-1">
                  <input
                    type="text"
                    placeholder={t('purchase.bills.print.itemDescription')}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
                ...
                <div className="w-24">
                  <input
                    type="number"
                    placeholder={t('purchase.bills.print.quantity')}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
                    value={item.quantity}
                ...
                <div className="w-32">
                  <input
                    type="number"
                    placeholder={t('purchase.bills.print.rate')}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 rtl:text-right"
                    value={item.rate}
          ...
          <div>
            ...
            <div className="mt-1">
              <textarea
                ...
                name="notes"
                rows={3}
                className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md border p-2 rtl:text-right"
