...
export const CreditNotePrintTemplate: React.FC<CreditNotePrintTemplateProps> = ({ data }) => {
    ...
    <div className="bg-white p-8 max-w-[210mm] mx-auto min-h-[297mm] text-sm text-gray-900 font-sans" dir={document.dir}>
      ...
      <div className="flex justify-between items-start border-b border-gray-200 pb-6 mb-8 rtl:flex-row-reverse">
        ...
        <div className="text-right rtl:text-left">
          <h2 className="text-2xl font-bold text-indigo-600 uppercase tracking-wide">{t('sales.creditNotes.print.title')}</h2>
          ...
          <div className="mt-2 inline-block px-3 py-1 bg-gray-100 border border-gray-200 rounded text-xs font-semibold uppercase text-gray-700">
            {t(`sales.creditNotes.status.${data.status}`)}
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between mb-8 bg-gray-50 p-6 rounded-lg border border-gray-100 rtl:flex-row-reverse">
        <div className="w-1/2 rtl:text-right">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('sales.creditNotes.print.creditTo')}</h3>
        ...
        <div className="w-1/2 text-right rtl:text-left">
          <div className="mb-2">
            <span className="text-gray-500 text-xs font-medium uppercase mr-2 rtl:ml-2 rtl:mr-0">{t('sales.creditNotes.print.date')}:</span>
            <span className="font-bold text-gray-900">{data.date}</span>
          </div>
          <div>
            <span className="text-gray-500 text-xs font-medium uppercase mr-2 rtl:ml-2 rtl:mr-0">{t('sales.creditNotes.print.referenceInvoice')}:</span>
      ...
      <table className="w-full mb-8 border-collapse">
        <thead>
          <tr className="bg-gray-100 border-y border-gray-200">
            ...
            <th className="py-3 px-4 text-left rtl:text-right font-bold text-gray-700 text-xs uppercase tracking-wider">{t('sales.creditNotes.print.itemDescription')}</th>
            <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-24 text-xs uppercase tracking-wider">{t('sales.creditNotes.print.quantity')}</th>
            <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-32 text-xs uppercase tracking-wider">{t('sales.creditNotes.print.rate')}</th>
            <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-32 text-xs uppercase tracking-wider">{t('sales.creditNotes.print.amount')}</th>
          </tr>
        </thead>
        <tbody>
      ...
      <div className="flex justify-end mb-12 rtl:justify-start">
        <div className="w-1/3">
          ...
          <div className="flex justify-between py-3 border-b-2 border-gray-200 rtl:flex-row-reverse">
            <span className="text-gray-800 text-sm font-bold uppercase">{t('sales.creditNotes.print.totalCredit')}</span>
      ...
      <div className="mt-auto">
        <div className="mb-8 text-xs text-gray-500 rtl:text-right">
          <h4 className="font-bold text-gray-700 uppercase mb-2">{t('sales.creditNotes.print.termsConditions')}</h4>
          <p>1. {t('sales.creditNotes.print.terms1')}</p>
          <p>2. {t('sales.creditNotes.print.terms2')}</p>
        </div>
        ...
        <div className="flex justify-between items-end pt-8 border-t border-gray-200 rtl:flex-row-reverse">
          <div className="w-1/3 text-center">
            ...
            <p className="font-bold text-gray-900 text-xs uppercase">{t('sales.creditNotes.print.authorizedBy')}</p>
          </div>
          <div className="w-1/3 text-center">
            <div className="h-20 border-b border-gray-300 mb-2"></div>
            <p className="font-bold text-gray-900 text-xs uppercase">{t('sales.creditNotes.print.receivedBy')}</p>
          </div>
        </div>
      </div>
    </div>
