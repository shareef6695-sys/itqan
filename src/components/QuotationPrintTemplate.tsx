...
import type { QuotationItem } from '../pages/sales/Quotations';

// Define interface locally to match the page's interface if not exported
...
export const QuotationPrintTemplate: React.FC<QuotationPrintTemplateProps> = ({ data }) => {
    ...
    <div className="bg-white p-8 max-w-[210mm] mx-auto min-h-[297mm] text-sm text-gray-900 font-sans" dir={document.dir}>
      ...
      <div className="flex justify-between items-start border-b border-gray-200 pb-6 mb-8 rtl:flex-row-reverse">
        ...
        <div className="text-right rtl:text-left">
          <h2 className="text-2xl font-bold text-indigo-600 uppercase tracking-wide">{t('sales.quotations.print.title')}</h2>
          ...
          <div className="mt-2 inline-block px-3 py-1 bg-gray-100 border border-gray-200 rounded text-xs font-semibold uppercase text-gray-700">
            {t(`sales.quotations.status.${data.status}`)}
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between mb-8 bg-gray-50 p-6 rounded-lg border border-gray-100 rtl:flex-row-reverse">
        <div className="w-1/2 rtl:text-right">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('sales.quotations.print.quotationFor')}</h3>
        ...
        <div className="w-1/2 text-right rtl:text-left">
          <div className="mb-2">
            <span className="text-gray-500 text-xs font-medium uppercase mr-2 rtl:ml-2 rtl:mr-0">{t('sales.quotations.print.date')}:</span>
            <span className="font-bold text-gray-900">{data.date}</span>
          </div>
          <div>
            <span className="text-gray-500 text-xs font-medium uppercase mr-2 rtl:ml-2 rtl:mr-0">{t('sales.quotations.print.validUntil')}:</span>
      ...
      <table className="w-full mb-8 border-collapse">
        <thead>
          <tr className="bg-gray-100 border-y border-gray-200">
            ...
            <th className="py-3 px-4 text-left font-bold text-gray-700 text-xs uppercase tracking-wider rtl:text-right">{t('sales.quotations.print.itemDescription')}</th>
            <th className="py-3 px-4 text-right font-bold text-gray-700 w-24 text-xs uppercase tracking-wider rtl:text-left">{t('sales.quotations.print.quantity')}</th>
            <th className="py-3 px-4 text-right font-bold text-gray-700 w-32 text-xs uppercase tracking-wider rtl:text-left">{t('sales.quotations.print.rate')}</th>
            <th className="py-3 px-4 text-right font-bold text-gray-700 w-32 text-xs uppercase tracking-wider rtl:text-left">{t('sales.quotations.print.amount')}</th>
          </tr>
        </thead>
        <tbody>
          {items.length > 0 ? (
      ...
      <div className="flex justify-end mb-12 rtl:justify-start">
        <div className="w-1/3">
          <div className="flex justify-between py-2 border-b border-gray-100 rtl:flex-row-reverse">
            <span className="text-gray-600 text-xs font-medium">{t('common.subtotal') || 'Subtotal'}</span>
            <span className="font-bold text-gray-900">{settings.currency} {subtotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between py-2 border-b border-gray-100 rtl:flex-row-reverse">
            <span className="text-gray-600 text-xs font-medium">{t('common.tax') || 'Tax'} ({settings.vatRate}%)</span>
          ...
          <div className="flex justify-between py-3 border-b-2 border-gray-200 rtl:flex-row-reverse">
            <span className="text-gray-800 text-sm font-bold uppercase">{t('common.total') || 'Total'}</span>
      ...
      <div className="mt-auto">
        <div className="mb-8 text-xs text-gray-500 rtl:text-right">
          <h4 className="font-bold text-gray-700 uppercase mb-2">{t('sales.quotations.print.termsConditions')}</h4>
          <p>1. {t('sales.quotations.print.terms1')}</p>
          <p>2. {t('sales.quotations.print.terms2')}</p>
          <p>3. {t('sales.quotations.print.terms3')}</p>
        </div>
        ...
        <div className="flex justify-between items-end pt-8 border-t border-gray-200 rtl:flex-row-reverse">
          <div className="w-1/3 text-center">
            ...
            <p className="font-bold text-gray-900 text-xs uppercase">{t('sales.quotations.print.salesRep')}</p>
          </div>
          <div className="w-1/3 text-center">
            <div className="h-20 border-b border-gray-300 mb-2"></div>
            <p className="font-bold text-gray-900 text-xs uppercase">{t('sales.quotations.print.clientAcceptance')}</p>
          </div>
        </div>
      </div>
    </div>
