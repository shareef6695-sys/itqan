...
import type { DebitNoteItem } from '../pages/purchase/DebitNotes';
...
export const DebitNotePrintTemplate: React.FC<DebitNotePrintTemplateProps> = ({ data }) => {
    ...
    <div className="bg-white p-8 max-w-[210mm] mx-auto min-h-[297mm] text-sm text-gray-900 font-sans" id="print-content" dir={document.dir}>
      ...
      <div className="flex justify-between items-start border-b border-gray-200 pb-6 mb-8 rtl:flex-row-reverse">
        ...
        <div className="text-right rtl:text-left">
          <h2 className="text-2xl font-bold text-indigo-600 uppercase tracking-wide">{t('purchase.debitNotes.print.title')}</h2>
      ...
      <div className="flex justify-between mb-8 bg-gray-50 p-6 rounded-lg border border-gray-100 rtl:flex-row-reverse">
        <div className="w-1/2 rtl:text-right">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t('purchase.debitNotes.print.vendor')}</h3>
        ...
        <div className="w-1/2 text-right rtl:text-left">
          <div className="mb-2">
            <span className="text-gray-500 text-xs font-medium uppercase mr-2">{t('purchase.debitNotes.print.date')}:</span>
            <span className="font-bold text-gray-900">{data.date}</span>
          </div>
          <div>
            <span className="text-gray-500 text-xs font-medium uppercase mr-2">{t('purchase.debitNotes.print.refBill')}:</span>
            <span className="font-bold text-gray-900">{data.billReference}</span>
          </div>
        </div>
      </div>
      ...
      <table className="w-full mb-8 border-collapse">
        <thead>
          <tr className="bg-gray-100 border-y border-gray-200">
            ...
            <th className="py-3 px-4 text-left rtl:text-right font-bold text-gray-700 text-xs uppercase tracking-wider">{t('purchase.debitNotes.print.itemDescription')}</th>
            <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-24 text-xs uppercase tracking-wider">{t('purchase.debitNotes.print.quantity')}</th>
            <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-32 text-xs uppercase tracking-wider">{t('purchase.debitNotes.print.rate')}</th>
            <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-32 text-xs uppercase tracking-wider">{t('purchase.debitNotes.print.amount')}</th>
          </tr>
        </thead>
        <tbody>
          {items.length > 0 ? (
